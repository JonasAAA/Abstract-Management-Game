...
namespace Game1
{
    ...
    public class CustomTexture : BaseTexture
    {
        ...
        private void SetColor(Point pos, Color color)
        {
            if (!editing)
                throw new InvalidOperationException();
            minEditX = Math.Min(minEditX, pos.X);
            maxEditX = Math.Max(maxEditX, pos.X);
            minEditY = Math.Min(minEditY, pos.Y);
            maxEditY = Math.Max(maxEditY, pos.Y);
